...
def read_xml(file_content: str):
    ...
    try:
        ...
        # Example: Convert XML to dictionary (Modify based on your needs)
    ...
    for position_elem in root.findall('Position'):
        ...
        if bluetooth is not None:
            for device in bluetooth.findall('Device'):
                name = device.find('Name')  # Using 'n' as it appears in the XML
                address_elem = device.find('Address')
                rssi_elem = device.find('RSSI')
                
                if name is not None and address_elem is not None and rssi_elem is not None:
                    position_data["Bluetooth"].append({
                        "Name": name.text,
                        "Address": address_elem.text,
                        "RSSI": int(rssi_elem.text)
